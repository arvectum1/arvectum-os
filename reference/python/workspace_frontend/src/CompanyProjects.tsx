...
export function CompanyProjects() {
  ...
  return <section className="company-page" aria-labelledby="company-projects-title">
    <header className="company-page-head"><p className="eyebrow">F11B · Product Contract Provisional 0.1.0</p><h1 id="company-projects-title">{text("Проекты компании", "Company projects")}</h1><p>{text("Read-only сводка по зарегистрированным проектам. Статус берётся только из канонического roadmap каждого проекта на точном commit SHA и фиксируется вместе с SHA-256 использованного содержимого.", "Read-only view of registered projects. Status comes only from each project's canonical roadmap at an exact commit SHA and is pinned with the SHA-256 of the content used.")}</p><p className="boundary-note">{text("Эта страница не изменяет roadmap, не запускает удалённое выполнение и не выдаёт права или Organizational Authority. Если repository/roadmap не reconciled, карточка показывает это явно вместо выдуманного locator/status.", "This page does not change roadmaps, invoke remote execution, or grant permission or Organizational Authority. If a repository or roadmap is not reconciled, the card says so instead of inventing a locator or status.")}</p><button type="button" onClick={refresh}>{text("Обновить из канонических источников", "Refresh canonical sources")}</button></header>
