...
export function Activity() {
  ...
  return <section className="activity-page" aria-labelledby="activity-title">
    <div className="my-work-heading"><div><p className="eyebrow">{text("Активность · производная проекция", "Activity · derived projection")}</p><h1 id="activity-title">{text("Операционная активность и сигналы", "Operational activity and alerts")}</h1><p>{text("Понятные наблюдения поверх уже авторизованных проекций рабочего пространства.", "Human-readable observations over already-authorized Workspace projections.")}</p></div><button type="button" className="quiet-button" onClick={() => void refresh()}>{text("Обновить", "Refresh")}</button></div>
