...
const starters = {
  ru: ["Что известно о текущем контексте Tender Operator?", "Что известно о Discount Parser?", "Найди информацию по закупке 0344100006426000005.", "Какие источники подтверждают этот контекст?"],
  en: ["What is known about the current Tender Operator context?", "What is known about Discount Parser?", "Find information about procurement notice 0344100006426000005.", "Which sources support this context?"],
};
...
export function Copilot({ csrfToken }: { csrfToken: string }) {
  ...
  return <section className="copilot" aria-labelledby="copilot-title">
    <div className="copilot-heading"><p className="eyebrow">Arvectum AI</p><h1 id="copilot-title">{text("Спросите Arvectum о текущем рабочем контексте", "Ask Arvectum about the current work context")}</h1><p>{text("Задайте вопрос обычным языком и откройте источники, на которых построен ответ.", "Ask in natural language and inspect the sources behind the answer.")}</p><p className="boundary-note">{text("Ответ отделяет контекст источников, вывод ИИ, неопределённость и недоступные свидетельства. Он временный и не создаёт Знание, разрешение или полномочие.", "The answer separates source context, AI synthesis, uncertainty, and unavailable evidence. It is transient and creates no Knowledge, permission, or authority.")}</p></div>
