...
export function Products({ productId }: { productId?: string | null }) {
  ...
  return <section className="products-section" aria-labelledby="products-title">
    <div className="my-work-heading"><div><p className="eyebrow">{text("Продуктовые контексты", "Product contexts")}</p><h2 id="products-title">{text("Продукты", "Products")}</h2><p>{text("Откройте доступный проверенный контекст продукта. Детальные операции остаются в самом продукте.", "Open an available verified product context. Detailed work remains in the product.")}</p></div></div>
